...
- (id)init
{
  ...
  if ((self = [super init])) {
    _instance = self;
    self.shareDialogMode = NativeDialogModes::AUTOMATIC;
    ...
    if ([FBSDKAccessToken currentAccessToken]) {
      ...
      [FBUnityUtility sendMessageToUnity:FBUnityMessageName_OnLoginComplete
                                userData:@{
                                           ...
                                           @"granted_permissions" : [result.grantedPermissions allObjects],
                                           @"declined_permissions" : [result.declinedPermissions allObjects]
                                           }
                               requestId:0];
      return;
    } else {
                      ...
                      photoURL:(const char *)photoURL
{
  ...
  dialog.mode = [self getDialogMode];
  FBUnitySDKDelegate *delegate = [FBUnitySDKDelegate instanceWithRequestID:requestId];
  dialog.delegate = delegate;
...
- (FBSDKShareDialogMode)getDialogMode
{
  switch (self.shareDialogMode) {
    case NativeDialogModes::ShareDialogMode::AUTOMATIC:
      return FBSDKShareDialogModeAutomatic;
    case NativeDialogModes::ShareDialogMode::NATIVE:
      return FBSDKShareDialogModeNative;
    case NativeDialogModes::ShareDialogMode::WEB:
      return FBSDKShareDialogModeWeb;
    case NativeDialogModes::ShareDialogMode::FEED:
...
extern "C" {
  ...
  {
    [FBUnityInterface sharedInstance].shareDialogMode = static_cast<NativeDialogModes::ShareDialogMode>(mode);
  }
  ...
  void iosGetDeepLink() {
    ...
    NSDictionary *dict = @{ @"deep_link" : url };
